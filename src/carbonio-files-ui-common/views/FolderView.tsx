...
const FolderView = (): React.JSX.Element => {
	...
	const createDocsFileAction = useCallback(
		(docsType: DocsType) => (_parentId: string, newName: string) => {
			if (currentFolder?.getNode && isFolder(currentFolder.getNode)) {
				return createDocsFile(currentFolder?.getNode, newName, docsType).then((result) => {
		...
		() =>
			[
				...
				...(canUseDocs
					...
					: [])
			] satisfies ContextualMenuProps['actions'],
		[canUseDocs, createDocsAction, createFolderAction, isCanCreateFile, isCanCreateFolder, t]
	);

	useEffect(() => {
		const createActions = actions.map<CreateOption>((action) => ({
