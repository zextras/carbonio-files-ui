...
describe('Versioning', () => {
	...
	test('delete version', async () => {
		...
		expect(screen.queryByText(getChipLabel(version2.last_editor))).not.toBeInTheDocument();
	});
	...
	test('purge all versions', async () => {
		...
		await waitFor(() => expect(screen.queryByTestId(SELECTORS.modal)).not.toBeInTheDocument());
		await waitFor(() => expect(screen.getAllByText(/Version \d/)).toHaveLength(2));
		expect(screen.queryByText(getChipLabel(version3.last_editor))).not.toBeInTheDocument();
	});
	...
	test('open with docs version', async () => {
		...
		expect(openNodeWithDocsSpy).toHaveBeenCalledWith(fileVersion1.id, fileVersion1.version);
	});
	...
	test('clone action is disabled if max number of version is reached', async () => {
		...
		const maxVersions = Number(
			configs.find((config) => config.name === CONFIGS.MAX_VERSIONS)?.value || 0
		);
	...
	test('keep forever action is disabled if max number of keep is reached', async () => {
		...
		const maxKeepVersions = Number(
			configs.find((config) => config.name === CONFIGS.MAX_KEEP_VERSIONS)?.value || 0
		);
		const baseFile = populateFile();
		baseFile.permissions.can_write_file = true;
	...
	test('upload version action is enabled if max number of versions is reached', async () => {
		...
		expect(await screen.findByText(RegExp(`version ${versionUpload.version}`, 'i'))).toBeVisible();
	...
	test('purge all is enabled if max number of versions is reached', async () => {
		...
		const purgedVersions = versions.slice(1).map((version) => version.version);
		const mocks = {
			Query: {
				getConfigs: mockGetConfigs(),
				getVersions: mockGetVersions(versions as FilesFile[])
		...
		const purgeAllModalButton = within(screen.getByTestId(SELECTORS.modal)).getByRole('button', {
			name: /purge all versions/i
		});
		await user.click(purgeAllModalButton);
